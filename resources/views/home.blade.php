    ...
    @if ($posts->count())
        @foreach ($posts as $keyPostDatum => $datumPost)
            <tr>
                ...
                <td> <a style="text-decoration:none;" href="{{ URL::to($postUrl) . '/' . $datumPost->id }}" >{{ $datumPost->title ?? '' }}</a></td>
                ...
                <td>
                    <a class="btn btn-sm btn-primary" href="{{ URL::to($postUrl) . '/' . $datumPost->id }}">Read more..</a>
                </td>
